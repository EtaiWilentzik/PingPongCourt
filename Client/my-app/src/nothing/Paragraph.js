...
const Paragraph = ({ heading, content, id }) => {
  return (
    <>
      <h4 className="spacing" id={id}>
        {heading}
      </h4>
      <p className="paragraph-content" id={`${id}-content`}>
        {content}
      </p>{" "}
      {/* adding id for the p tag */}
    </>
  );
};
...
const ParagraphsContainer = () => {
  const paragraphs = [
    {
      heading: "About",
      content: "hello 1 asdfasdfadsfasdfasdfasdf ",
  ...
  return (
    <>
      <nav
        id="navbar-example2"
        className="navbar bg-body-tertiary px-3 mb-3 fixedElement  "
      >
      ...
      <div
        ...
        className="scrollspy-example bg-body-tertiary p-3 rounded-2 av"
        tabIndex="0"
      >
        {paragraphs.map((para, index) => (
          <Paragraph
