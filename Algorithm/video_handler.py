...
class VideoWriter:
    def __init__(self, output_path, frame_size, fps):
        ...
        if not self.out.isOpened():
            raise IOError(
                f"Cannot open video file for writing: {self.output_path}")
        logging.debug(
            f'VideoWriter initialized: {self.output_path}, Frame Size: {self.frame_size}, FPS: {self.fps}')

    def write_frame(self, frame):
        if frame is not None:
            self.out.write(frame)
            logging.debug('Frame written to video file')
        else:
            logging.warning('Received None frame, skipping write')
